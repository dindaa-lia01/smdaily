...
<div class="bg-gray-100 py-6 md:py-8 mb-8 md:mb-10 w-full">
    <div class="max-w-[1536px] mx-auto px-6 lg:px-16">
        <h1 class="text-2xl md:text-4xl font-bold text-gray-900 mb-1 md:mb-2 tracking-tight">Pembangunan</h1>
        <p class="text-sm md:text-lg text-gray-700 max-w-2xl">Liputan mendalam mengenai infrastruktur, tata ruang kota, dan inovasi pembangunan yang membentuk wajah baru daerah kita hari ini dan esok.</p>
    </div>
</div>
...
<div class="max-w-[1536px] mx-auto px-6 lg:px-16 pb-10">
    ...
    @if($heroBerita)
    <div class="mb-10 md:mb-16 bg-white border border-gray-100 rounded-2xl md:rounded-3xl p-4 md:p-6 shadow-sm">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5 md:gap-8 items-center">
            <div class="relative h-[200px] sm:h-[250px] md:h-[300px] lg:h-[400px] rounded-xl md:rounded-2xl overflow-hidden">
                <img src="{{ asset('assets/images/berita/'.$heroBerita->gambar_thumbnail) }}" class="w-full h-full object-cover">
                <span class="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-gray-900 text-[10px] font-bold uppercase px-3 py-1 rounded shadow-sm">
                    {{ $heroBerita->kategori->nama_kategori ?? 'Pembangunan' }}
                </span>
            </div>
            <div class="flex flex-col justify-center">
                <div class="flex items-center gap-3 text-xs text-gray-400 font-medium mb-2 md:mb-3 uppercase">
                    <span>{{ $heroBerita->created_at->diffForHumans() }}</span>
                    <span class="text-gray-300">|</span>
                    <span class="flex items-center gap-1.5"
                          id="views-card-{{ $heroBerita->id_berita }}">
                        <span class="views-text">{{ number_format($heroBerita->views ?? 0, 0, ',', '.') }}</span> Views
                    </span>
                </div>
                <a href="{{ route('berita.detail', $heroBerita->slug_berita) }}" class="group block">
                    <h1 class="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 mb-3 md:mb-4 leading-tight group-hover:text-indigo-600 transition-colors duration-300">
                        {{ $heroBerita->judul_berita }}
                    </h1>
                </a>
                <p class="text-sm text-gray-600 mb-4 md:mb-6 line-clamp-3">{{ Str::limit(strip_tags($heroBerita->isi_berita), 200) }}</p>
            </div>
        </div>
    </div>
    @endif
...
{{-- FILTER BAR (Tombol Toggle & Rata Kanan) --}}
            ...
            <article class="{{ $viewMode == 'list' ? 'flex flex-row w-full' : 'flex flex-col h-full' }} bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition duration-300">
                <div class="{{ $viewMode == 'list' ? 'w-2/5 sm:w-1/3 min-h-[130px] sm:h-48' : 'w-full aspect-[16/10]' }} relative overflow-hidden flex-shrink-0">
                    ...
                    <span class="absolute top-3 left-3 bg-white/90 backdrop-blur-sm text-gray-900 text-[8px] font-bold uppercase px-1.5 py-0.5 rounded shadow-sm tracking-wider">
                        {{ $b->kategori->nama_kategori ?? 'Pembangunan' }}
                    </span>
                </div>
...
<script>
/**
 * SINKRONISASI VIEWS CARD — pembangunan.blade.php
